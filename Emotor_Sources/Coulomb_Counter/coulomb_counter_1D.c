...
#elif defined(_CELL_TYPE_SDI_26F_) && (_CELL_SERIES_NUMBER_ == 16)
    #include "OCV_Table_SDI_26F_16S.h"
////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////
//  SDI 26H
...
// Coulomb Counter FCC Updated in CHG Voltage
    ...
    #define CHG_Update_Vol_TH   64000   //mV, when voltage > this th, fit update condition
#else
...
////////////////////////////////////////////////////
// add by hsinmo-20140613
#define CallingTimingBase_mS                100     //100ms will be called once
#define OneMinuteTimeCountTh_ByTimeBase     600     // 1min = 600 * 100ms
unsigned int gTimeCallingCount;
unsigned int gTimeMinutesCount;
unsigned long gAccumulatingDSG_ADC;
unsigned long gAccumulatingCHG_ADC;
void init_calculation(){
    gAccumulatingDSG_ADC = 0;
    gAccumulatingCHG_ADC = 0;
    gTimeCallingCount = 0;
    gTimeMinutesCount = 0;
}
unsigned long getRealDSGCurrent_mA(unsigned long adc_Val){
    return (unsigned long)(adc_Val/g_Factor_Values[Param_DSG_Index]);
}
unsigned long getRealCHGCurrent_mA(unsigned long adc_Val){
    return (unsigned long)(adc_Val/g_Factor_Values[Param_CHG_Index]);
}
unsigned long getRealVoltage_mV(unsigned long adc_Val){
    return (unsigned long)(adc_Val/g_Factor_Values[Param_VBAT_Index]);
}
////////////////////////////////////////////////////
// add by hsinmo-20140613
void calculation(unsigned char current_status){

    gTimeCallingCount++;
    if(gTimeCallingCount >= OneMinuteTimeCountTh_ByTimeBase){
        gTimeMinutesCount++;
        if(gTimeMinutesCount >= 0xfff9){
            gTimeMinutesCount = 0xfff9;
        }
    }

    if(current_status == CurrentInDSG){
        gAccumulatingDSG_ADC += g_ADC_Values[Param_DSG_Index]; //每100ms累加一次dsg_adc_value
    }else if(current_status == CurrentInCHG){
        gAccumulatingCHG_ADC += g_ADC_Values[Param_CHG_Index]; //每100ms累加一次chg_adc_value
    }else{
        //Current In Static
    }
}
