/**
  ******************************************************************************
  * @file    DAC/DAC_SignalsGeneration/stm8l15x_it.c
...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
extern CONST uint16_t WavesTab[];
uint8_t  TIM4PresTab[TIM4_PRESTAB_SIZE] =
  {
...
INTERRUPT_HANDLER(EXTI1_IRQHandler, 9)
{
  ...
  if (STM_EVAL_PBGetState(BUTTON_LEFT) == RESET)
  {
    ...
    DMA_Init(DMA1_Channel3,  WavesTab[WavesTabIndex++], DAC_CH1RDHRH_ADDRESS,
...
INTERRUPT_HANDLER(EXTI3_IRQHandler, 11)
{
  ...
  {
    TIM4_TimeBaseInit(TIM4_Prescaler_1,  TIM4PresTab[TIM4PresTabIndex]);

    TIM4PresTabIndex++;

    if (TIM4PresTabIndex == TIM4_PRESTAB_SIZE)
    {
      TIM4PresTabIndex = 0;
    }
